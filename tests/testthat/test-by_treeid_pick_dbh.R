context("by_treeid_pick_dbh_max")
...
describe("by_treeid_pick_dbh_max()", {
  it("outputs the same groups as input", {
    # Ungrouped
    collapsed <- by_treeid_pick_dbh_max(cns)
    expect_equal(group_vars(collapsed), group_vars(cns))
    
    # Grouped
    bysp <- group_by(cns, sp)
    collapsed <- by_treeid_pick_dbh_max(bysp)
  ...
  it("automatically groups by CensusID", {
    ...
    out <- by_treeid_pick_dbh_max(.cns)
    ...
    out2 <- by_treeid_pick_dbh_max(.cns2)
  ...
  it("drops missing values of censusid if there are multiple unique censusid", {
    ...
    expect_silent(out <- by_treeid_pick_dbh_max(cns))
...
describe("by_treeid_pick_dbh_*()", {
  it("picks only one row per treeid per census despite ties", {
    ...
    out <- by_treeid_pick_dbh_min(cns)
