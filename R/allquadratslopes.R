...
#' @keywords internal
allquadratslopes <- 
function (elev, gridsize = 20, plotdim = c(1000, 500), edgecorrect = TRUE) 
{
    ...
    elevdata = elev$col[elev$col$x%%gridsize == 0 & elev$col$y%%gridsize == 
        0, ]
    elevmat = matrix(elevdata$elev, nrow = rows, ncol = columns, 
        byrow = F)
    meanelev = convex = convex2 = slope = numeric()
    corner = sideht = numeric()
    for (c in 1:(columns - 1)) for (r in 1:(rows - 1)) {
        quad.index = rowcol_to_index(r, c, gridsize = gridsize, 
            plotdim = plotdim)
    ...
    for (i in 1:totalquads) {
        neighbor.quads = findborderquads(i, dist = gridsize, 
    ...
    if (edgecorrect) {
        for (c in 1:(columns - 1)) for (r in 1:(rows - 1)) {
            if ((c == 1) | (c == (columns - 1)) | (r == 1) | 
                ...
                quad.index = rowcol_to_index(r, c, gridsize = gridsize, 
                  plotdim = plotdim)
                xy = index_to_gxgy(quad.index, gridsize = gridsize, 
                  plotdim = plotdim)
...
#' @keywords internal
findborderquads <- 
function (index, dist = 20, gridsize = 20, plotdim = c(1000, 
    500)) 
{
    ...
    for (i in (row - layers):(row + layers)) for (j in (col - 
        ...
        if (i >= 1 & i <= maxrow & j >= 1 & j <= maxcol) {
            ...
            bound.index[no.boundaries] = rowcol_to_index(i, j, 
                gridsize, plotdim)
        }
...
#' @keywords internal
quadslope <- 
function (cornerelev, gridsize = 20) 
{
    slope = numeric(4)
    z = numeric(3)
    for (j in 1:4) {
